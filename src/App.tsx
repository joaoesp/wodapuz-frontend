...
import { ACTIVE_CONFLICTS } from "./conflictData";
...
const METRIC_DESCRIPTIONS: Record<string, string> = {
  ...
  "Active Conflicts":
    "Major active armed conflicts worldwide. Hover a conflict icon to highlight involved countries on the map; click to view a detailed summary.",
  "Energy Production":
...
const NO_LEGEND_METRICS = new Set([
  "Military Alliances",
  "Active Conflicts",
  "Energy Infrastructure",
]);
...
function App() {
  ...
  const [hoveredConflict, setHoveredConflict] = useState<ActiveConflict | null>(null);
  const [openConflict, setOpenConflict] = useState<ActiveConflict | null>(null);
    ...
    <div className="app">
      <WorldMap
        ...
        highlightedCountries={
          selectedMetric === "Active Conflicts" ? (hoveredConflict?.countries ?? []) : undefined
        }
      />
      ...
      {openConflict && (
        <ConflictDashboard conflict={openConflict} onClose={() => setOpenConflict(null)} />
      )}
