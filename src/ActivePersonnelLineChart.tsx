...
interface ActivePersonnelLineChartProps {
  startYear: number;
  endYear: number;
  onClose: () => void;
}
...
function formatValue(value: number): string {
  return value >= 1e6 ? `${(value / 1e6).toFixed(2)}M` : `${(value / 1e3).toFixed(0)}K`;
}

function ActivePersonnelLineChart({ startYear, endYear, onClose }: ActivePersonnelLineChartProps) {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(
          `http://localhost:1337/api/world-bank/active-personnel/years/${startYear}/${endYear}`
        );

        if (!response.ok) {
          throw new Error("Failed to fetch active personnel data");
        }
        ...
        countries.sort((a, b) => {
          const aLatest = a.dataPoints[a.dataPoints.length - 1]?.value || 0;
          const bLatest = b.dataPoints[b.dataPoints.length - 1]?.value || 0;
        ...
        console.error("Error fetching active personnel data:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const width = 700;
  const height = 400;
  const padding = { top: 20, right: 150, bottom: 40, left: 75 };
  ...
  return (
    <div className="gdp-line-chart">
      <div className="chart-header">
        <h3 className="chart-title">Active Military Personnel — G20</h3>
...
export default ActivePersonnelLineChart;
