...
interface DebtToGdpLineChartProps {
  startYear: number;
  endYear: number;
  onClose: () => void;
}
...
function DebtToGdpLineChart({ startYear, endYear, onClose }: DebtToGdpLineChartProps) {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(
          `http://localhost:1337/api/world-bank/debt-to-gdp/years/${startYear}/${endYear}`
        );

        if (!response.ok) {
          throw new Error("Failed to fetch Debt-to-GDP data");
        }
  ...
  const maxValue = Math.ceil(Math.max(...allValues) / 50) * 50;
  const minValue = 0;
  ...
  return (
    <div className="gdp-line-chart">
      <div className="chart-header">
        <h3 className="chart-title">Government Debt (% of GDP)</h3>
...
export default DebtToGdpLineChart;
