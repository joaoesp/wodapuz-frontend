...
interface CurrentAccountBalanceLineChartProps {
  startYear: number;
  endYear: number;
}
...
function CurrentAccountBalanceLineChart({
  startYear,
  endYear,
}: CurrentAccountBalanceLineChartProps) {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(
          `http://localhost:1337/api/world-bank/current-account-balance/years/${startYear}/${endYear}`
        );
        if (!response.ok) throw new Error("Failed to fetch Current Account Balance data");
        ...
        // Sort by average absolute value descending, take top 10
        countries.sort((a, b) => {
          const aAvg =
        ...
        console.error("Error fetching Current Account Balance data:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className={`gdp-line-chart ${isMinimized ? "minimized" : ""}`}>
      <div className="chart-header">
        <h3 className="chart-title">Current Account Balance (% of GDP)</h3>
        <button
...
export default CurrentAccountBalanceLineChart;
