...
const SOURCES = [
  ...
  { key: "otherRenewable" as const, label: "Other", color: "#6b7280" },
];
...
function TrendChart({ years, values }: { years: number[]; values: (number | null)[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: "block" }}>
      ...
      {data.map((d) => (
        <circle key={d.year} cx={x(d.year)} cy={y(d.value)} r={2} fill="#4ecdc4" />
      ))}
    </svg>
  );
}
...
function EnergyConsumptionDashboard({
  countryCode,
  countryName,
  selectedYear,
  onClose,
}: EnergyConsumptionDashboardProps) {
  const [data, setData] = useState<CountryConsumptionData | null>(null);
      ...
      <div className="energy-dashboard" onClick={(e) => e.stopPropagation()}>
        <div className="energy-dashboard-header">
          <div>
            ...
            <p className="energy-dashboard-subtitle">Energy Consumption</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="energy-dashboard-charts">
            <div className="energy-chart-section">
              <h4 className="energy-section-label">Primary Energy Consumption</h4>
