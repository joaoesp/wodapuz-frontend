...
interface GdpPerCapitaLineChartProps {
  startYear: number;
  endYear: number;
  onClose: () => void;
}
...
function GdpPerCapitaLineChart({ startYear, endYear, onClose }: GdpPerCapitaLineChartProps) {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(
          `http://localhost:1337/api/world-bank/gdp-per-capita/years/${startYear}/${endYear}`
        );

        if (!response.ok) {
          throw new Error("Failed to fetch GDP per capita data");
        }
        ...
        // Sort by latest year value descending, take top 10
        countries.sort((a, b) => {
  ...
  return (
    <div className="gdp-line-chart">
      <div className="chart-header">
        <h3 className="chart-title">GDP per Capita Over Time</h3>
...
export default GdpPerCapitaLineChart;
